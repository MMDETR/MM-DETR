...
class LoadCCImageFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        results['cc_img'] = img
        results['cc_img_shape'] = img.shape[:2]
        results['cc_ori_shape'] = img.shape[:2]
        if True:
            # Choose a flip direction
            # Options: 'horizontal', 'vertical', 'diagonal'
            flip_direction = random.choice(['horizontal', 'vertical', 'diagonal'])
            
            # Flip the supporting image using mmcv
            results['cc_img'] = mmcv.imflip(
                results['cc_img'], direction=flip_direction)
            
            results['cc_img_shape'] = results['cc_img'].shape[:2]
            results['cc_ori_shape'] = results['cc_img'].shape[:2]
        return results
