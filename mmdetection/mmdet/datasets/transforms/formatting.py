...
def resize(img, target_shape):
    ...
    return F.interpolate(img.float().unsqueeze(0), size=(target_h, target_w), mode='bilinear', align_corners=False).squeeze(0)
...
class PackDetInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        """
        packed_results = dict()
        if 'cc_img' in results:
            self.meta_keys = ('img_id', 'img_path', 'ori_shape', 'img_shape',
                            'scale_factor', 'flip', 'flip_direction',
                            'text', 'custom_entities' ,
                            'cc_path', 'cc_ori_shape', 'cc_img_shape')
            mlo_img = results['img']
            cc_img = results['cc_img']
            if len(mlo_img.shape) < 3:
                mlo_img = np.expand_dims(mlo_img, -1)
            if len(cc_img.shape) < 3:
                cc_img = np.expand_dims(cc_img, -1)    
            ...
            # for more details
            if not mlo_img.flags.c_contiguous:
                mlo_img = np.ascontiguousarray(mlo_img.transpose(2, 0, 1))
                mlo_img = to_tensor(mlo_img)
            else:
                mlo_img = to_tensor(mlo_img).permute(2, 0, 1).contiguous()

            if not cc_img.flags.c_contiguous:
                cc_img = np.ascontiguousarray(cc_img.transpose(2, 0, 1))
                cc_img = to_tensor(cc_img)
            else:
                cc_img = to_tensor(cc_img).permute(2, 0, 1).contiguous()

            #max_h = max(mlo_img.shape[1], cc_img.shape[1])
            #max_w = max(mlo_img.shape[2], cc_img.shape[2])

            h, w = mlo_img.shape[1], mlo_img.shape[2]
            cc_img = resize(cc_img, (h, w))
            # Pad both images to the same shape
            #mlo_img = pad_to_match(mlo_img, (max_h, max_w))
            #cc_img = pad_to_match(cc_img, (max_h, max_w))
            
            inputs = torch.stack([mlo_img, cc_img], dim=0)
